...
public class TurnRightSpeedActionTest extends PhysicsBaseTest {
	...
	private void initRightSpeedValue(float speed) {
		...
		Action action = sprite.getActionFactory().createTurnRightSpeedAction(sprite, null, new Formula(speed));
	...
	public void testBrickWithStringFormula() {
		PhysicsObject physicsObject = physicsWorld.getPhysicsObject(sprite);
		sprite.getActionFactory().createTurnRightSpeedAction(sprite, null, new Formula(String.valueOf(SPEED))).act(1.0f);
		assertEquals("Unexpected rotation speed value", -SPEED, physicsObject.getRotationSpeed());

		sprite.getActionFactory().createTurnRightSpeedAction(sprite, null, new Formula(
	...
	public void testNullFormula() {
		PhysicsObject physicsObject = physicsWorld.getPhysicsObject(sprite);
		sprite.getActionFactory().createTurnRightSpeedAction(sprite, null, null).act(1.0f);
		assertEquals("Unexpected rotation speed value", -0f, physicsObject.getRotationSpeed());
	}

	public void testNotANumberFormula() {
		PhysicsObject physicsObject = physicsWorld.getPhysicsObject(sprite);
		sprite.getActionFactory().createTurnRightSpeedAction(sprite, null, new Formula(Double.NaN)).act(1.0f);
		assertEquals("Unexpected rotation speed value", 0f, physicsObject.getRotationSpeed());
	}
}
