...
public class SetVolumeToActionTest extends InstrumentationTestCase {
	...
	public void testVolume() {
		sprite.getActionFactory().createSetVolumeToAction(sprite, null, volume).act(1.0f);
	...
	public void testBrickWithStringFormula() {
		sprite.getActionFactory().createSetVolumeToAction(sprite, null, new Formula(String.valueOf(VOLUME))).act(1.0f);
		assertEquals("Incorrect sprite volume value after SetVolumeToBrick executed", VOLUME, SoundManager.getInstance()
				.getVolume());

		sprite.getActionFactory().createSetVolumeToAction(sprite, null, new Formula(String.valueOf(NOT_NUMERICAL_STRING))).act(1.0f);
		assertEquals("Incorrect sprite volume value after SetVolumeToBrick executed", VOLUME, SoundManager.getInstance()
				.getVolume());
	}

	public void testNullFormula() {
		sprite.getActionFactory().createSetVolumeToAction(sprite, null, null).act(1.0f);
		assertEquals("Incorrect sprite volume value after SetVolumeToBrick executed", 0f, SoundManager.getInstance()
				.getVolume());
	}

	public void testNotANumberFormula() {
		sprite.getActionFactory().createSetVolumeToAction(sprite, null, new Formula(Double.NaN)).act(1.0f);
		assertEquals("Incorrect sprite volume value after SetVolumeToBrick executed", VOLUME, SoundManager.getInstance()
				.getVolume());
	}
}
