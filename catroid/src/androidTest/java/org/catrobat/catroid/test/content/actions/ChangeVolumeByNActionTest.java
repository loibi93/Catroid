...
public class ChangeVolumeByNActionTest extends InstrumentationTestCase {
	...
	public void testVolume() {
		...
		sprite.getActionFactory().createChangeVolumeByNAction(sprite, null, louder).act(1.0f);
		...
		sprite.getActionFactory().createChangeVolumeByNAction(sprite, null, softer).act(1.0f);
		assertEquals("Incorrect sprite size value after ChangeVolumeByNBrick executed", expectedVolume, SoundManager
				.getInstance().getVolume());
	}

	public void testBrickWithStringFormula() {
		sprite.getActionFactory().createChangeVolumeByNAction(sprite, null, new Formula(String.valueOf(CHANGE_VALUE)))
				.act(1.0f);
		...
		sprite.getActionFactory().createChangeVolumeByNAction(sprite, null, new Formula(NOT_NUMERICAL_STRING)).act(1.0f);
		assertEquals("Incorrect sprite volume after ChangeVolumeByNBrick executed", INITIALIZED_VALUE + CHANGE_VALUE,
				SoundManager.getInstance().getVolume());
	}

	public void testNullFormula() {
		sprite.getActionFactory().createChangeVolumeByNAction(sprite, null, null).act(1.0f);
		assertEquals("Incorrect sprite volume after ChangeVolumeByNBrick executed", INITIALIZED_VALUE, SoundManager
				.getInstance().getVolume());
	}

	public void testNotANumberFormula() {
		sprite.getActionFactory().createChangeVolumeByNAction(sprite, null, new Formula(Double.NaN)).act(1.0f);
		assertEquals("Incorrect sprite volume after ChangeVolumeByNBrick executed", INITIALIZED_VALUE, SoundManager
				.getInstance().getVolume());
	}
}
