...
public class ChangeYByNActionTest extends AndroidTestCase {
	...
	public void testNormalBehavior() {
		...
		sprite.getActionFactory().createChangeYByNAction(sprite, null, new Formula(CHANGE_VALUE)).act(1.0f);
		assertEquals("Incorrect sprite y position after ChangeYByNBrick executed", CHANGE_VALUE,
				sprite.look.getYInUserInterfaceDimensionUnit());
	}

	public void testNullSprite() {
		Action action = sprite.getActionFactory().createChangeYByNAction(null, null, new Formula(CHANGE_VALUE));
		try {
			action.act(1.0f);
	...
	public void testBoundaryPositions() {
		...
		sprite.getActionFactory().createChangeYByNAction(sprite, null, new Formula(Integer.MAX_VALUE)).act(1.0f);
		...
		sprite.getActionFactory().createChangeYByNAction(sprite, null, new Formula(Integer.MIN_VALUE)).act(1.0f);
		assertEquals("ChangeYByNBrick failed to place Sprite at minimum y integer value", Integer.MIN_VALUE,
				(int) sprite.look.getYInUserInterfaceDimensionUnit());
	}

	public void testBrickWithStringFormula() {
		sprite.getActionFactory().createChangeYByNAction(sprite, null, new Formula(String.valueOf(CHANGE_VALUE))).act(1.0f);
		assertEquals("Incorrect sprite y position after ChangeYByNBrick executed", CHANGE_VALUE,
				sprite.look.getYInUserInterfaceDimensionUnit());

		sprite.getActionFactory().createChangeYByNAction(sprite, null, new Formula(NOT_NUMERICAL_STRING)).act(1.0f);
		assertEquals("Incorrect sprite y position after ChangeYByNBrick executed", CHANGE_VALUE,
				sprite.look.getYInUserInterfaceDimensionUnit());
	}

	public void testNullFormula() {
		sprite.getActionFactory().createChangeYByNAction(sprite, null, null).act(1.0f);
		assertEquals("Incorrect sprite y position after ChangeYByNBrick executed", 0f,
				sprite.look.getYInUserInterfaceDimensionUnit());
	}

	public void testNotANumberFormula() {
		sprite.getActionFactory().createChangeYByNAction(sprite, null, new Formula(Double.NaN)).act(1.0f);
		assertEquals("Incorrect sprite y position after ChangeYByNBrick executed", 0f,
				sprite.look.getYInUserInterfaceDimensionUnit());
	}
}
