...
public class ChangeSizeByNActionTest extends InstrumentationTestCase {
	...
	public void testSize() {
		...
		sprite.getActionFactory().createChangeSizeByNAction(sprite, null, new Formula(CHANGE_SIZE)).act(1.0f);
	...
	public void testNullSprite() {
		...
		Action action = factory.createChangeSizeByNAction(null, null, new Formula(CHANGE_SIZE));
		try {
			action.act(1.0f);
	...
	public void testBrickWithStringFormula() {
		sprite.getActionFactory().createChangeSizeByNAction(sprite, null, new Formula(String.valueOf(CHANGE_VALUE)))
				.act(1.0f);
		assertEquals("Incorrect sprite size value after ChangeSizeByNBrick executed", INITIALIZED_VALUE + CHANGE_VALUE,
				sprite.look.getSizeInUserInterfaceDimensionUnit());

		sprite.getActionFactory().createChangeSizeByNAction(sprite, null, new Formula(NOT_NUMERICAL_STRING)).act(1.0f);
		assertEquals("Incorrect sprite size value after ChangeSizeByNBrick executed", INITIALIZED_VALUE + CHANGE_VALUE,
				sprite.look.getSizeInUserInterfaceDimensionUnit());
	}

	public void testNullFormula() {
		sprite.getActionFactory().createChangeSizeByNAction(sprite, null, null).act(1.0f);
		assertEquals("Incorrect sprite size value after ChangeSizeByNBrick executed", INITIALIZED_VALUE,
				sprite.look.getSizeInUserInterfaceDimensionUnit());
	}

	public void testNotANumberFormula() {
		sprite.getActionFactory().createChangeSizeByNAction(sprite, null, new Formula(Double.NaN)).act(1.0f);
		assertEquals("Incorrect sprite size value after ChangeSizeByNBrick executed", INITIALIZED_VALUE,
				sprite.look.getSizeInUserInterfaceDimensionUnit());
	}
}
