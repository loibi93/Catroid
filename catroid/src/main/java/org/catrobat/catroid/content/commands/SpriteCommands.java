...
public class SpriteCommands {
	...
	public static class AddSpriteCommand implements Command {
		...
		public void execute() {
			...
			if (currentBackground != null) {
				backgroundSpriteBackup = ProjectManager.getInstance().getCurrentScene().replaceBackgroundSprite(currentBackground);
			}
		...
		@Override
		public void undo() {
			for (PointToBrick brick : ProjectManager.getInstance().getCurrentScene().getPointToBricks()) {
			...
			for (Sprite sprite : sprites) {
				ProjectManager.getInstance().getCurrentScene().removeSprite(sprite);
			}

			if (backgroundSpriteBackup != null) {
				currentBackground = ProjectManager.getInstance().getCurrentScene().replaceBackgroundSprite(backgroundSpriteBackup);
			}
		}
	}
	...
	public static class DeleteSpriteCommand implements Command {
		...
		public void execute() {
			...
			for (Sprite sprite : sprites) {
				...
				positions.add(ProjectManager.getInstance().getCurrentScene().getSpritePositionById(sprite));
			}
			ProjectManager.getInstance().getCurrentScene().getSpriteList().removeAll(sprites);
		}

		@Override
		public void undo() {
			for (int i = 0; i < sprites.size(); i++) {
				ProjectManager.getInstance().getCurrentScene().getSpriteList().add(positions.get(i), sprites.get(i));
			}
	...
	public static class MoveSpriteCommand implements Command {
		...
		@Override
		public void execute() {
			List<Sprite> lookList = ProjectManager.getInstance().getCurrentScene().getSpriteList();
